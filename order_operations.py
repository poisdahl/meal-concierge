...
class OrderOperations:
    ...
    def _checkout_payment_closed(pending):
        ...
        if isinstance(child, Mapping) and child.get("status") == "awaiting_confirmation":
            ...
            if (child.get("prior_vipps_request_status") == "expired"
                    and not OrderOperations._vipps_expiry_proven(retained_source or {})):
                return False
        ...
        vipps_failure = method == "vipps" and OrderOperations._vipps_expiry_proven(attempt)
    ...
    def _checkout_operation(self, request: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if "checkout_payment" in request and action not in {"prepare", "switch_payment"}:
            raise HouseholdError("checkout_payment override is available only during preparation or an explicit payment switch")
        if "identity_review" in request and (
                action != "prepare" or not isinstance(request["identity_review"], Mapping)
                or request.get("recovery") or request.get("occurrence") or request.get("scheduler")
                or self.provider not in {"oda", "mathem"}):
            raise HouseholdError("identity_review is available only for a manual Oda/Mathem new-checkout preparation")
        ...
        if "owner_payment_completed" in request and not (
            action == "reconcile" and request.get("owner_payment_completed") is True
            and not request.get("vipps_approval_completed")
            and not request.get("vipps_request_not_received")
        ):
            raise HouseholdError("owner_payment_completed is true only for exact externally completed payment reconciliation")
        ...
        if (action == "prepare" and request.get("recovery") and request.get("order_id") is not None
            ...
            raise HouseholdError("Exact Oda recovery requires the original or current confirmation and the owner's current no-request report")
        ...
        if "order_id" in request and not (
            ...
            legacy_result = None
            if action == "confirm":
                legacy_result = self._read_protected_result(
                    state, str(request.get("confirmation_id") or ""), "checkout"
                )
            elif action == "submit":
                key = self._idempotency_key(request.get("idempotency_key"), "checkout")
                protected = self._protected_request(state, "checkout", key)
                legacy_result = protected.get("result") if isinstance(protected, Mapping) else None
        ...
        if action == "prepare":
            ...
            return self._checkout_prepare(
                ...
                identity_review=request.get("identity_review"),
            )
        if action == "confirm":
        ...
        if action == "reconcile":
            return self._checkout_reconcile(
                ...
                owner_payment_completed=request.get("owner_payment_completed") is True,
            )
    ...
    def _checkout_prepare(
        ...
        *,
        ...
        _switch_confirmation_id: str | None = None,
        identity_review: Mapping[str, Any] | None = None,
    ) -> dict[str, Any]:
        if identity_review is not None and (
                self.provider not in {"oda", "mathem"} or self.browser is None
                or not isinstance(identity_review, Mapping) or occurrence or automatic_checkout
                or scheduler_context or cart_ready_continuation):
            raise HouseholdError("identity_review requires a manual Oda/Mathem new checkout with its configured browser")
        ...
        with self.store.locked() as state:
            if identity_review is not None and (
                    state.get("order_change") or (state.get("pending_checkout") or {}).get("occurrence")
                    or (state.get("pending_checkout") or {}).get("automatic_checkout")
                    or (state.get("pending_checkout") or {}).get("scheduler_context")):
                raise HouseholdError("identity_review cannot resolve an addition or scheduled checkout")
            if state.get("pending_cart_change"):
            ...
            if (_switch_confirmation_id is not None
                    and (not baseline or baseline.get("confirmation_id") != _switch_confirmation_id
                         or baseline.get("status") != "awaiting_confirmation" or baseline.get("recovery"))):
                raise HouseholdError("The checkout review changed before switching payment")
        ...
        with self._browser_operation(deadline):
                ...
                if self.provider == "meny":
                    ...
                    try:
                        review = self.browser.review_checkout(cart, deadline=deadline,
                            **({"payment": checkout_payment} if self.provider == "oda" else {}),
                            **({"identity_review": deepcopy(identity_review)} if identity_review is not None else {}))
            ...
            if self.provider == "meny":
                ...
                if after_delivery is not None:
                    ...
                    observed_window = oda_delivery_signature(after_delivery.get("display"), provider=self.provider)
                    if self.provider == "oda" and observed_window is None:
                        # Oda's cart uses "Hjemlevering mellom kl 13 og 18, i morgen".
                        # Its existing exact parser binds relative labels to Oslo's date.
                        window = oda_cart_delivery_window(after_delivery, today=self._now().astimezone(ZoneInfo("Europe/Oslo")).date())
                        if window["date"] == order_change["before"]["order"].get("deliveryDate"):
                            observed_date = date.fromisoformat(window["date"])
                            months = ("jan", "feb", "mar", "apr", "mai", "jun", "jul", "aug", "sep", "okt", "nov", "des")
                            observed_window = (*map(int, window["start"].split(":")), *map(int, window["end"].split(":")), observed_date.day, months[observed_date.month - 1])
                    if (expected_window is None
                            or observed_window != expected_window
    ...
    @staticmethod
    def _vipps_expiry_proven(attempt):
        context = attempt.get("vipps_request_context") or {}
        digest = context.get("gateway_url_digest")
        return (isinstance(digest, str) and bool(re.fullmatch(r"[0-9a-f]{64}", digest))
                and attempt.get("vipps_expiry_gateway_digest") == digest)

    @staticmethod
    def _vipps_reported_unsent(attempt, order_id):
        # A subsequently discovered gateway does not mean Next was clicked.
        # The current attempt's own durable dispatch fence and owner report
        # determine whether its independently checked unsent proof still holds.
        return bool(
            attempt.get("vipps_request_status") == "not_sent"
            and all(attempt.get(key) is None for key in (
                "vipps_request_attempted_at", "payment_requested_at", "owner_vipps_approval_completed_at"))
            and isinstance(attempt.get("owner_reported_no_vipps_request_after_attempt_at"), str)
            and attempt["owner_reported_no_vipps_request_after_attempt_at"]
            and attempt.get("payment_failure") == {
                "payment_failed": True, "order_id": order_id,
                "reason": "owner_reported_no_vipps_request_before_dispatch_fence",
            }
        )

    @staticmethod
    def _closed_payment_switch(pending):
        for attempt in (pending.get("recovery") or {}, pending):
            switch = attempt.get("payment_switch") or {}
            if (switch.get("closure") or {}).get("status") == "closed":
                return switch
        return None

    @staticmethod
    def _switch_closes_active_attempt(pending):
        child = pending.get("recovery")
    ...
    def _recovery_old_request_allows_dispatch(self, pending, child):
        ...
        if (child.get("prior_vipps_request_status") == "expired"
            ...
            return False
        if (pending.get("checkout_payment") or {}).get("method") != "vipps":
            return True
        original_status = pending.get("vipps_request_status")
        prior_recovery_closed = (
            original_status is None
            and child.get("prior_vipps_request_status") in {"verifying", "not_sent"}
        )
        legacy_offer = (
            original_status is None
        ...
        return (
            original_status in {"verifying", "not_sent"}
            or (original_status == "expired" and self._vipps_expiry_proven(pending))
    ...
    def _adopt_checkout_vipps_request(self, pending, order_id, deadline):
        ...
        with self.store.locked() as state:
            if canonical(state.get("pending_checkout")) != canonical(pending):
                raise HouseholdError("The payment changed during gateway adoption")
            target = state["pending_checkout"]["recovery"] if child else state["pending_checkout"]
            target["vipps_request_context"] = deepcopy(context)
            # Finding the hosted request does not prove whether Next ran or
            # the provider closed it. Earlier phone reports or page timeouts
            # must not authorize another payment while we observe this one.
            target["vipps_request_status"] = "unknown"
            target.pop("vipps_expiry_gateway_digest", None)
            if (target.get("payment_failure") or {}).get("reason") in {
                    "owner_reported_no_vipps_request_before_dispatch_fence", "vipps_request_expired"}:
                target.pop("payment_failure", None)
            state["pending_checkout"]["unpaid_order_id"] = order_id
            state["pending_checkout"]["unpaid_order_binding_source"] = "oda_reconciled_exact_order"
    ...
    def _checkout_abort_payment(self, deadline, confirmation_id):
        ...
        with self._browser_operation(deadline):
            ...
            if method == "vipps" and not isinstance(context, Mapping):
                ...
                attempt = pending.get("recovery") or pending
                if not isinstance(context, Mapping):
    ...
    def _checkout_switch_payment(self, deadline, confirmation_id, checkout_payment):
        ...
        if not isinstance(confirmation_id, str) or not confirmation_id:
            raise HouseholdError("Payment switching requires the exact current confirmation_id")
        if not isinstance(checkout_payment, Mapping):
            raise HouseholdError("Payment switching requires the requested checkout_payment")
        payment = checkout_payment_settings(checkout_payment, self.provider)
        preview = self.store.read().get("pending_checkout") or {}
        if preview.get("status") == "awaiting_confirmation" and not preview.get("recovery"):
            if confirmation_id != preview.get("confirmation_id"):
                raise HouseholdError("Payment switching must identify the current checkout review")
            # Preparation owns its browser lock; its journal comparison keeps
            # this pre-dispatch selection tied to the supplied current review.
            return self._checkout_prepare(deadline, payment_override=payment,
        ...
        with self._browser_operation(deadline):
            ...
            pending = deepcopy(state.get("pending_checkout"))
            if not pending:
                raise HouseholdError("No Oda checkout is pending")
            child = pending.get("recovery")
            if child and child.get("status") == "awaiting_confirmation":
                switch = child.get("payment_switch") or {}
                current_review = confirmation_id == child.get("confirmation_id")
                replay = (confirmation_id == switch.get("source_confirmation_id")
                          and switch.get("checkout_payment") == payment
                          and child.get("browser_review", {}).get("payment_choice") == payment)
                if not (current_review or replay):
                    raise HouseholdError("Payment switching must identify the current recovery review")
                if (child.get("browser_review", {}).get("payment_choice") == payment
                        and self._now() < datetime.fromisoformat(child["expires_at"])):
                    return self._recovery_prepared_view(pending)
                return self._checkout_recovery_prepare_unlocked(deadline, payment, _switch_payment=current_review)
            if pending.get("status") not in UNRESOLVED_CHECKOUT_STATUSES:
                raise HouseholdError("No dispatched Oda payment is pending")
            source_child = bool(child)
            attempt = child if source_child else pending
            method = ((attempt.get("browser_review") or {}).get("payment_choice")
                      if source_child else pending.get("checkout_payment")) or {}
            if confirmation_id != attempt.get("confirmation_id"):
                raise HouseholdError("Payment switching must identify the current payment attempt")
            ...
            switch = inherited if inherited.get("source_confirmation_id") == confirmation_id else None
            if switch and switch.get("checkout_payment") != payment:
                raise HouseholdError("A different payment switch is already in progress")

            def update_switch(changes):
                nonlocal pending, switch
                with self.store.locked() as locked:
                    if canonical(locked.get("pending_checkout")) != canonical(pending):
                        raise HouseholdError("The payment attempt changed during switching")
                    target = locked["pending_checkout"]["recovery"] if source_child else locked["pending_checkout"]
                    target["payment_switch"] = {**(switch or {}), **deepcopy(changes)}
                    pending = deepcopy(locked["pending_checkout"])
                    switch = deepcopy((pending["recovery"] if source_child else pending)["payment_switch"])

            if not switch:
                update_switch({"source_confirmation_id": confirmation_id, "checkout_payment": payment,
                               "status": "closing", "started_at": self._now().isoformat()})

            def before_cancel(evidence):
                if switch.get("cancel_attempted"):
                    raise HouseholdError("The Vipps cancellation was already attempted; observe it without repeating")
            ...
            if (switch.get("closure") or {}).get("status") != "closed":
                ...
                update_switch({"closure": closure, "status": "closed" if closure.get("status") == "closed" else "unresolved"})
                if closure.get("status") == "paid":
                    result = self._checkout_reconcile_unlocked(deadline, confirmation_id)
                    return {**result, "payment_switch_pending": not result.get("confirmed"),
                            "next": "The original payment reports acceptance. Reconcile it; do not send a replacement payment."}
                if closure.get("status") != "closed":
                    return {"confirmed": False, "confirmation_id": confirmation_id, "payment_switch_pending": True,
                            ...
                            "next": "The original payment is not proven closed. Resume switch_payment with this same confirmation to observe it; no replacement payment has been sent."}
            if change:
                target = self.browser.prepare_oda_addition_retry(change["order_id"], pending["cart"], change["before"]["order"],
                    change["binding"], deadline=deadline, closure=deepcopy(switch["closure"]),
                    retained_target=deepcopy(switch.get("target")))
                if (not isinstance(target, Mapping) or target.get("order_id") != change["order_id"]
                        or not isinstance(target.get("order_change_id"), str) or not target["order_change_id"]):
                    return {"confirmed": False, "confirmation_id": confirmation_id, "payment_switch_pending": True,
                            "retry_allowed": False, "recovery_preparation_available": False,
    ...
    def _checkout_recovery_target(self, pending, deadline, requested_order_id=None):
        ...
        if requested is not None and (
            ...
            raise HouseholdError("An exact recovery order_id is supported only for an Oda/Vipps new order")
        if pending.get("order_change"):
            change = pending["order_change"]
            failure = pending.get("payment_failure") or {}
            switch = self._closed_payment_switch(pending) if self.provider == "oda" else None
            target = (switch or {}).get("target") or {}
            oda_target = bool(switch and target.get("order_id") == change["order_id"] and target.get("order_change_id"))
            if (change.get("requested_delivery") or not (oda_target or (
                    self.provider in {"oda", "mathem"} and failure.get("payment_failed") is True
                    and failure.get("order_id") == change["order_id"] and failure.get("order_change_id")))):
        ...
        if (self.provider == "oda"
                ...
                and (retained_order_id is None
                     or binding_source not in ODA_VIPPS_ORDER_BINDING_SOURCES)
                and requested is None):
            raise HouseholdError(
                "Recovery has no exact Oda order identity; supply the order_id only after "
                "the owner identifies that order as Betaling påbegynt"
            )
        ...
        active_recovery = pending.get("recovery")
        owner_reported_no_request = bool(
            requested is not None
            or (
                pending.get("vipps_request_status") == "not_sent"
                and pending.get("owner_reported_no_vipps_request_after_attempt_at")
                and pending.get("payment_failure") == {
                    "payment_failed": True, "order_id": order_id,
                    "reason": "owner_reported_no_vipps_request_before_dispatch_fence",
                }
            )
            or (
                isinstance(active_recovery, Mapping)
                and active_recovery.get("order_id") == order_id
                and (
                    (
                        active_recovery.get("owner_reported_no_vipps_request") is True
                        and active_recovery.get("original_confirmation_id") == pending.get("confirmation_id")
                    )
                    or (
                        active_recovery.get("vipps_request_status") == "not_sent"
                        and isinstance(active_recovery.get("owner_reported_no_vipps_request_after_attempt_at"), str)
                        and bool(active_recovery["owner_reported_no_vipps_request_after_attempt_at"])
                        and active_recovery.get("payment_failure") == {
                            "payment_failed": True,
                            "order_id": order_id,
                            "reason": "owner_reported_no_vipps_request_before_dispatch_fence",
                        }
        ...
        current_payment = ((current_attempt.get("browser_review") or {}).get("payment_choice")
                           if current_attempt is not pending else pending.get("checkout_payment")) or {}
        contextless_legacy = (
            self.provider == "oda" and current_payment.get("method") == "vipps"
            and current_attempt.get("vipps_request_status") in {None, "not_sent"}
            and current_attempt.get("vipps_request_context") is None
            and current_attempt.get("vipps_request_attempted_at") is None
    ...
    def _checkout_recovery_prepare(self, deadline, checkout_payment=None, requested_order_id=None,
                                   original_confirmation_id=None):
        with self._browser_operation(deadline):
            return self._checkout_recovery_prepare_unlocked(
                deadline, checkout_payment, requested_order_id, original_confirmation_id
            )
    ...
    def _checkout_recovery_prepare_unlocked(self, deadline, checkout_payment=None,
        ...
        current_child = pending.get("recovery") or {}
        if (requested_order_id is not None
                and original_confirmation_id != pending.get("confirmation_id")
                and not (original_confirmation_id == current_child.get("confirmation_id")
                         and requested_order_id == current_child.get("order_id") == pending.get("unpaid_order_id"))):
            raise HouseholdError("Exact Oda recovery does not match the original or current checkout confirmation")
        active_attempt = pending.get("recovery") or pending
        if (active_attempt.get("vipps_request_status") == "expired"
                and not self._vipps_expiry_proven(active_attempt)):
            result = self._checkout_reconcile_unlocked(deadline, active_attempt["confirmation_id"])
            pending = deepcopy(self.store.read().get("pending_checkout"))
            if not pending or result.get("confirmed"):
                return result
            active_attempt = pending.get("recovery") or pending
        legacy_retry_offer = (
        ...
        switch = self._closed_payment_switch(pending)
        if (switch and child and child.get("status") != "awaiting_confirmation"
                and switch.get("source_confirmation_id") != child.get("confirmation_id")):
            # The earlier switch selected this attempt's method. Once this
            # attempt itself has failed, that selection does not constrain
            # recovery of its independently verified outcome.
            switch = None
        payment = checkout_payment_settings(checkout_payment, self.provider) if checkout_payment is not None else (switch["checkout_payment"] if switch else pending["checkout_payment"])
        if switch and payment != switch["checkout_payment"] and not _switch_payment:
            raise HouseholdError("This completed payment switch can prepare only its requested method; use switch_payment on the current review to change it")
        ...
        carried_owner_no_request = bool(
            self.provider == "oda"
            and pending.get("unpaid_order_binding_source") in ODA_VIPPS_ORDER_BINDING_SOURCES
            and isinstance(previous, Mapping) and previous.get("order_id") == order_id
            and self._vipps_reported_unsent(previous, order_id)
        )
        ...
        if (self.provider == "oda" and not pending.get("order_change")
                and self._checkout_payment_closed(pending)):
            inherited_source = (source_attempt.get("closed_source")
                                if source_attempt.get("status") == "awaiting_confirmation" else None)
            child["closed_source"] = deepcopy(inherited_source or {
                ...
                "vipps_expiry_gateway_digest": source_attempt.get("vipps_expiry_gateway_digest"),
        ...
        if (requested_order_id is not None or carried_owner_no_request
                or pending.get("vipps_request_status") == "not_sent"
                and pending.get("owner_reported_no_vipps_request_after_attempt_at")):
    ...
    def _checkout_recovery_confirm(self, pending, deadline, request):
        ...
        with self._browser_operation(deadline):
            ...
            if self.store.read()["checkout_payment"] != pending.get("payment_preference", pending["checkout_payment"]):
            ...
            def on_vipps_gateway(context):
                ...
                with self.store.locked() as state:
                    if (not dispatch_claimed or vipps_prepared is not None
                            or canonical(state.get("pending_checkout")) != canonical(dispatched)):
                        raise HouseholdError("Recovery changed before retaining its Vipps gateway")
                    current = state["pending_checkout"]["recovery"]
                    current["vipps_request_status"] = "prepared"
                    current["vipps_request_context"] = deepcopy(context)
                    vipps_prepared = deepcopy(state["pending_checkout"])

            def before_vipps_request(context):
                nonlocal vipps_dispatched
                if (not isinstance(context, Mapping)
                        or context.get("order_id") != child["order_id"]):
                    raise HouseholdError("Recovery Vipps handoff belongs to a different Oda order")
                with self.store.locked() as state:
                    if (not dispatch_claimed
                            or canonical(state.get("pending_checkout")) != canonical(vipps_prepared or dispatched)
                            or vipps_prepared and context != vipps_prepared["recovery"]["vipps_request_context"]):
            ...
            try:
                submit_result = self.browser.submit_payment_recovery(
                    ...
                    **({"on_vipps_gateway": on_vipps_gateway} if self.provider == "oda"
                       and child["browser_review"]["payment_choice"]["method"] == "vipps" else {}),
                    **self._recovery_browser_options(pending),
                )
                ...
                with self.store.locked() as state:
                    expected_dispatched = vipps_dispatched or vipps_prepared or dispatched
                    ...
                    if vipps_request_sent:
                        if vipps_dispatched is None:
                            raise HouseholdError("The Oda/Vipps request has no durable dispatch acknowledgement")
                ...
                if not dispatch_claimed or isinstance(exc, RecoveryClickNotDispatchedError):
                    with self.store.locked() as state:
                        current = state.get("pending_checkout")
                        expected = (vipps_dispatched or vipps_prepared or dispatched) if dispatch_claimed else pending
        ...
        try:
            with self._browser_operation(deadline):
                ...
                def on_vipps_gateway(context) -> None:
                    ...
                    with self.store.locked() as state:
                        current = state.get("pending_checkout")
                        if vipps_prepared is not None or canonical(current) != canonical({**pending, "status": "clicking"}):
                            raise HouseholdError("Checkout changed before retaining its Vipps gateway")
                        current["vipps_request_status"] = "prepared"
                        current["vipps_request_context"] = deepcopy(context)
                        vipps_prepared = deepcopy(current)
                ...
                def before_vipps_request(context) -> None:
                    ...
                    with self.store.locked() as state:
                        current = state.get("pending_checkout")
                        expected = vipps_prepared or {**pending, "status": "clicking"}
                        if (not current or canonical(current) != canonical(expected)
                                or vipps_prepared and context != vipps_prepared["vipps_request_context"]):
                            raise HouseholdError("checkout changed before the Oda/Vipps request")
                        # The exact reviewed cart, one controlled Oda click and
                        # same-tab hosted page bind this attempt.
                        # Oda's order list can lag; identify the order during
                        # reconciliation instead of blocking the phone request.
                        current["vipps_request_status"] = "dispatching"
                        current["vipps_request_context"] = deepcopy(context)
                        current["vipps_request_attempted_at"] = self._now().isoformat()
                ...
                if evidence or pending.get("authentication_unresolved"):
                    with self.store.locked() as state:
                        expected_dispatched = vipps_dispatched or vipps_prepared or {**pending, "status": "clicking"}
    ...
    def _checkout_authentication_wait(self, pending, deadline):
        ...
        change = pending.get("order_change") or {}
        if (not observed and context and not change.get("requested_delivery")
                and hasattr(self.browser, "checkout_payment_failure")):
            failure = self.browser.checkout_payment_failure(context, deadline=deadline,
                **({"expected_order_id": change["order_id"]} if change else {}))
            if failure:
                if attempt is child:
                    target = (self._closed_payment_switch(pending) or {}).get("target") or {}
                    expected_change = target.get("order_change_id") or (pending.get("payment_failure") or {}).get("order_change_id")
                    if (failure.get("order_id") != child["order_id"]
                            or self.provider == "mathem" and failure != pending.get("payment_failure")
                ...
                with self.store.locked() as state:
                    ...
                    attempt.pop("authentication_unresolved", None)
                return {}
    ...
    def _checkout_reconcile(self, deadline: float | None = None, confirmation_id: str = "", *,
                            ...
                            vipps_request_not_received: bool = False,
                            owner_payment_completed: bool = False) -> dict[str, Any]:
        with self._browser_operation(deadline):
            return self._checkout_reconcile_unlocked(
                ...
                owner_payment_completed=owner_payment_completed,
            )
    ...
    def _checkout_reconcile_unlocked(self, deadline: float | None = None, confirmation_id: str = "", *,
        ...
        if pending.get("order_change"):
            if owner_payment_completed:
                raise HouseholdError("Owner payment completion requires the exact original Oda order")
            return self._order_change_reconcile(pending, deadline)
        legacy_child = pending.get("recovery")
        legacy_attempt = (legacy_child if legacy_child and legacy_child.get("status") != "awaiting_confirmation"
                          else pending)
        if (self.provider == "oda" and legacy_attempt.get("vipps_request_status") == "expired"
                and not self._vipps_expiry_proven(legacy_attempt)):
            # Older releases persisted a screen timeout as payment expiry.
            # Re-read that exact request before the old label can authorize
            # another payment. Unknown is not a terminal provider outcome.
            context = legacy_attempt.get("vipps_request_context")
            observed = self.browser.checkout_vipps_request_state(context, deadline=deadline)
            with self.store.locked() as state:
                if canonical(state.get("pending_checkout")) != canonical(pending):
                    raise HouseholdError("The payment changed while verifying its stored expiry")
                target = (state["pending_checkout"]["recovery"] if legacy_attempt is not pending
                          else state["pending_checkout"])
                if observed.get("status") == "expired" and isinstance(context, Mapping):
                    target["vipps_expiry_gateway_digest"] = context["gateway_url_digest"]
                else:
                    target["vipps_request_status"] = "sent" if observed.get("status") == "sent" else "unknown"
                    if (target.get("payment_failure") or {}).get("reason") == "vipps_request_expired":
                        target.pop("payment_failure", None)
                pending = deepcopy(state["pending_checkout"])
        if vipps_request_not_received:
            ...
            same_unsent_attempt = (
                self.provider == "oda"
                and confirmation_id == reported_attempt.get("confirmation_id")
                and reported_attempt.get("status") in {"clicking", "uncertain", "awaiting_user_payment"}
                and reported_payment.get("method") == "vipps"
                and reported_order_id and reported_order_id == pending.get("unpaid_order_id")
                and reported_attempt.get("vipps_request_attempted_at") is None
                and reported_attempt.get("payment_requested_at") is None
                and reported_attempt.get("owner_vipps_approval_completed_at") is None
            )
            context = reported_attempt.get("vipps_request_context")
            contextless_exact_recovery = (
                same_unsent_attempt
                and pending.get("unpaid_order_binding_source") == "oda_retry_available_page"
                and reported_attempt.get("vipps_request_status") is None and context is None
            )
            prior_report = (
                same_unsent_attempt
                and isinstance(reported_attempt.get("owner_reported_no_vipps_request_after_attempt_at"), str)
                and bool(reported_attempt["owner_reported_no_vipps_request_after_attempt_at"])
            )
            recorded_owner_no_request = (
                prior_report and context is None
                and pending.get("unpaid_order_binding_source") == "oda_retry_available_page"
                and reported_attempt.get("vipps_request_status") == "not_sent"
                and reported_attempt.get("payment_failure") == {
                    "payment_failed": True, "order_id": reported_order_id,
                    "reason": "owner_reported_no_vipps_request_before_dispatch_fence",
                }
            )
            adopted_unsent_report = (
            ...
            if adopted_unsent_report:
                # Adoption discovers the existing session; it cannot erase a
                # prior exact pre-Next observation. A fresh provider read must
                # not contradict that observation or the renewed owner report.
                observation = self.browser.checkout_vipps_request_state(context, deadline=deadline)
                adopted_unsent_report = observation.get("status") in {"unknown", "prepared"}
            if not (contextless_exact_recovery or recorded_owner_no_request or adopted_unsent_report):
                raise HouseholdError(
                    "vipps_request_not_received does not match an exact Oda attempt before request dispatch"
                )
            ...
            if not recorded_owner_no_request:
                self._checkout_recovery_target(pending, deadline, reported_order_id)
                with self.store.locked() as state:
                    if canonical(state.get("pending_checkout")) != canonical(pending):
                        raise HouseholdError("checkout state changed while recording the missing Vipps request")
                    current = state["pending_checkout"]["recovery"] if isinstance(child, Mapping) else state["pending_checkout"]
                    ...
                    current["payment_failure"] = {
                        "payment_failed": True,
                        "order_id": safe_order_id(reported_order_id),
                        "reason": "owner_reported_no_vipps_request_before_dispatch_fence",
                    }
                    pending = deepcopy(state["pending_checkout"])
        ...
        if owner_payment_completed and (
            self.provider != "oda"
            or not confirmation_id
            or confirmation_id != active_attempt.get("confirmation_id")
            or not pending.get("unpaid_order_id")
            or not (pending.get("browser_review") or {}).get("account_reference_digest")
        ):
            raise HouseholdError("Owner payment completion requires the current confirmation, exact Oda order and original account binding")
        ...
        if self.provider == "oda" and active_payment.get("method") == "vipps":
            if active_attempt is pending:
                original_context = pending.get("vipps_request_context")
                oda_vipps_bound = (
                    pending.get("unpaid_order_binding_source") in ODA_VIPPS_ORDER_BINDING_SOURCES
                    or (
                        pending.get("vipps_request_status") in {"prepared", "dispatching", "sent"}
                        and isinstance(original_context, Mapping)
                        and original_context.get("order_id") is None
                    )
                )
            else:
        ...
        if self.provider == "oda" and active_payment.get("method") == "vipps" and not owner_payment_completed:
            request_status = active_attempt.get("vipps_request_status")
            if request_status in {"prepared", "dispatching", "sent", "unknown"}:
                context = active_attempt.get("vipps_request_context")
                observed = self.browser.checkout_vipps_request_state(context, deadline=deadline)
                oda_vipps_observation = str((observed or {}).get("status") or "unknown")
                if oda_vipps_observation == "unknown" and request_status == "prepared":
                    oda_vipps_observation = "prepared"
            ...
            oda_vipps_active = (not oda_vipps_expired and not oda_vipps_not_sent
                                and not active_attempt.get("payment_failure"))
        vipps_before_dispatch = (
            self.provider == "oda" and active_payment.get("method") == "vipps"
            and active_attempt.get("vipps_request_status") in {"prepared", "unknown"}
            and active_attempt.get("vipps_request_attempted_at") is None
            and oda_vipps_observation != "sent"
            and ((active_attempt.get("payment_switch") or {}).get("closure") or {}).get("status") != "paid"
        ...
        candidate_evidence = {}
        candidate_evidence_unresolved = False
        if (self.provider in {"oda", "mathem"} and exact_failed_order_id is None
                and retained_unpaid_id is None):
            matching_candidates = []
            for candidate in candidates:
                try:
                    candidate_id = safe_order_id(str(
                        candidate.get("orderNumber") or candidate.get("order_number")
                        or candidate.get("id") or ""
                    ))
                    details = self.provider_client.call(
                        "get_order", {"order_number": candidate_id}, deadline=deadline
                    )
                    candidate_tracking = self.provider_client.call(
                        "order_tracking", {"order_number": candidate_id}, deadline=deadline
                    )
                except HouseholdError:
                    candidate_evidence_unresolved = True
                    continue
                details_id = str(
                    details.get("orderNumber") or details.get("order_number")
                    or details.get("id") or ""
                )
                tracking_id = str(
                    candidate_tracking.get("orderNumber")
                    or candidate_tracking.get("order_number")
                    or candidate_tracking.get("order_id")
                    or candidate_tracking.get("id") or ""
                )
                addressed = details if "deliveryAddress" in details or "delivery_address" in details else {
                    **details,
                    "deliveryAddress": (pending["summary"].get("delivery") or {}).get("address"),
                }
                if (candidate_id == details_id == tracking_id
                        and order_matches_checkout(addressed, pending["summary"], provider=self.provider)):
                    matching_candidates.append(candidate)
                    candidate_evidence[candidate_id] = (details, candidate_tracking)
        ...
        if exact_failed_order_id is not None:
            ...
            tracking_id = str(tracking.get("orderNumber") or tracking.get("order_number") or tracking.get("order_id") or tracking.get("id") or "")
            order = {**(matching_rows[0] if matching_rows else {}), **details}
        if (exact_failed_order_id is None and retained_unpaid_id is None
                and len(candidates) == 1 and not candidate_ambiguous
                and not candidate_evidence_unresolved):
            candidate_id = str(candidates[0].get("orderNumber") or candidates[0].get("order_number") or candidates[0].get("id") or "")
            details, tracking = candidate_evidence.get(candidate_id) or (
                self.provider_client.call("get_order", {"order_number": candidate_id}, deadline=deadline),
                self.provider_client.call("order_tracking", {"order_number": candidate_id}, deadline=deadline),
            )
            details_id = str(details.get("orderNumber") or details.get("order_number") or details.get("id") or "")
            tracking_id = str(tracking.get("orderNumber") or tracking.get("order_number") or tracking.get("order_id") or tracking.get("id") or "")
        ...
        exact_not_sent_recovery = bool(
            isinstance(active_attempt, Mapping)
            and (active_attempt is pending or recovery_dispatched and active_attempt is pending.get("recovery"))
            and (pending.get("unpaid_order_id") if active_attempt is pending else active_attempt.get("order_id")) == pending.get("unpaid_order_id")
            and self._vipps_reported_unsent(active_attempt, pending.get("unpaid_order_id"))
        )
        ...
        exact_retry_review_matches = exact_not_sent_recovery and active_attempt is pending
        if exact_not_sent_recovery and active_attempt is not pending and tracking_conflict:
            try:
                current_review = self.browser.review_payment_recovery(
                    pending["cart"], candidate_id,
                    payment=active_attempt["browser_review"]["payment_choice"],
                    expected_binding=active_attempt["browser_review"]["binding"],
                    deadline=deadline,
                )
                exact_retry_review_matches = canonical(current_review) == canonical(active_attempt["browser_review"])
            except HouseholdError:
                pass
        receipt_order = order
        ...
        unpaid_matches = (
            ...
            and candidate_id == details_id == tracking_id
            and tracking_status == "unpaid_order"
        ...
        exact_payment_started_matches = (
            exact_not_sent_recovery
            and tracking_conflict
            and exact_retry_review_matches
            and order is not None
            and candidate_id == details_id == tracking_id
            and order_matches_checkout(
                order if "deliveryAddress" in order or "delivery_address" in order else {
        ...
        with self.store.locked() as state:
            ...
            if confirmed:
                ...
                terminal = {
                    ...
                    **({"owner_payment_completed": True} if owner_payment_completed else {}),
                ...
                if (unpaid_matches and self.provider in {"oda", "mathem"}
                    ...
                    if (self.provider == "oda"
                            and pending.get("unpaid_order_binding_source") not in ODA_VIPPS_ORDER_BINDING_SOURCES
                            and active_attempt is pending
                            and isinstance(pending.get("vipps_request_context"), Mapping)
                            and pending["vipps_request_context"].get("order_id") is None):
                        state["pending_checkout"]["unpaid_order_binding_source"] = "oda_reconciled_exact_order"
                if oda_vipps_active:
                    state["pending_checkout"]["status"] = "awaiting_user_payment"
                else:
                ...
                if oda_vipps_expired:
                    ...
                    context = target.get("vipps_request_context") or {}
                    if context.get("gateway_url_digest"):
                        target["vipps_expiry_gateway_digest"] = context["gateway_url_digest"]
        ...
        return {
            ...
            **({**({"unpaid_order_id": candidate_id} if unpaid_matches else {"recovery_order_id": candidate_id}),
                "tracking_status": tracking_status,
                "recovery_preparation_available": True,
                "next": ("The exact Oda order page still says payment started although tracking reports paid. Inspect its supported same-order retry review with checkout prepare recovery=true; do not recreate the cart or resubmit the original confirmation. A new review does not send payment."
                         if exact_payment_started_matches else
               ...
               if recovery_target_matches and oda_vipps_bound and not oda_vipps_active
            ...
            **({"awaiting_user_payment": oda_vipps_observation != "prepared",
                ...
                **({"unpaid_order_id": candidate_id} if unpaid_matches else {}),
                "next": ("The exact Vipps gateway is retained before its mobile request. An explicit switch_payment can close this same payment and prepare the requested method; do not submit another order."
                         if oda_vipps_observation == "prepared" else
                         "The original Oda/Vipps request may still be active. Approve it if present, or use switch_payment for an explicitly requested replacement. Reconcile this same attempt; never send another payment while its outcome is unknown.")}
    ...
    def _order_change_reconcile(self, pending: Mapping[str, Any], deadline: float | None = None) -> dict[str, Any]:
        ...
        if self.provider == "meny":
            ...
            fulfillable = status in {"paid_and_modifiable", "paid_and_not_modifiable", "picking", "shipped", "delivered"}
        attempt = pending["recovery"] if recovery_dispatched else pending
        before_vipps_dispatch = (
            self.provider == "oda" and attempt.get("vipps_request_status") in {"prepared", "unknown"}
            and attempt.get("vipps_request_attempted_at") is None
            and ((attempt.get("payment_switch") or {}).get("closure") or {}).get("status") != "paid"
        )
        confirmed = (current_id == tracking_id == order_id and matched and fulfillable
                     and (not before_vipps_dispatch or status in {"picking", "shipped", "delivered"}))
        authentication = self._checkout_authentication_wait(pending, deadline) if not confirmed else {}
        vipps_followup = {}
        vipps_status = None
        ...
        with self.store.locked() as state:
            ...
            if confirmed:
                ...
                if vipps_status is not None:
                    target_attempt = state["pending_checkout"]["recovery"] if recovery_dispatched else state["pending_checkout"]
                    target_attempt["vipps_request_status"] = vipps_status
