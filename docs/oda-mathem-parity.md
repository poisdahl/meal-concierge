...
| Browser account/receipt | Mathem's account reference and the exact cancelled test receipt's address matched independently. Oda redirected to login despite working MCP authentication | Mathem read binding verified; Oda authenticated browser acceptance blocked. No session repair or address-guard bypass |
...
A complete current-service trial through review remains outstanding.
...
Oda's missing order-address binding remains unresolved; its guard is preserved.
...
without UI preparation; supported price-changing review/authorization; current
Oda browser/account/order binding and affected native flow checks; candidate
publication, normal installation/upgrade and verification on exactly authorized
runtime/client targets. No new general order, payment, deployment, login-repair
