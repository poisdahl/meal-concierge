...
| Grok desktop 0.44.0 / tested scheduler installation 9599097 | Native scheduled saved-menu controls succeeded with app open and during measured app closure: reported due-to-start delays 442.311/397.448 seconds and reads 77.438/55.456 seconds after start. Correct results were observed before any controller wake. Exact routines were reconciled and paused. A separate 13 September retained-home update/start/attach reached current public `bb34755`; it did not run a scheduler occurrence. | Original silent occurrence and causal pause suppression remain unverified. Outer success/finish precedes the actual read and is not workflow latency. No physical host/VM sleep or reliability guarantee. The current-source update does not broaden these historical scheduler results. |
...
This scheduler result feeds MC10's owner-rescoped closure. It does not by itself
certify any of the broader platform boundaries explicitly excluded from
[#52](https://github.com/poisdahl/meal-concierge/issues/52).
