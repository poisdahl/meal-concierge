...
@server.tool(description='Explicitly save one complete recipe or frozen discovery, update an exact revision, or archive a built-in recipe. External update requires provider-enforced conditional write and an exact versioned library_recipe_ref. External archive/delete uses recipe_lifecycle. Keep a stable idempotency key for one intent; reconcile uncertain saves with the same key, never recreate them. New typed recipes use schema_version=2 and exact fraction quantities. accept_estimates accepts only server-resolved recipe_id/expected_revision or discovery_ref with its returned recipe_digest, exact estimate_fields and the explicit confirmation_statement: I accept these exact recipe estimates and their stated assumptions. Show estimates and assumptions first; never invent acceptance or source evidence. Acceptance creates a new version, retains estimate labels and creates no personal entry for discovery.')
...
def _menu_plan_projection(plan: dict[str, Any]) -> dict[str, Any]:
    """Keep decision evidence and complete handoffs without repeated plan views."""
    projected = {key: value for key, value in plan.items()
                 if key not in {"canonical_input", "selection", "selections", "save_handoffs"}}
    ...
    if plan.get("save_handoff") is not None:
        projected.pop("request", None)  # Already present in every complete handoff.
        projected["alternative_handoffs"] = plan["save_handoffs"][1:]
...
def meal_concierge_checkout(action: Literal["prepare", "confirm", "submit", "reconcile", "auto"] = "prepare", occurrence: str | None = None, confirmation_id: str | None = None, idempotency_key: str | None = None, scheduler: dict[str, Any] | None = None) -> dict[str, Any]:
    return rpc("checkout", action=action, occurrence=occurrence, confirmation_id=confirmation_id, idempotency_key=idempotency_key, scheduler=scheduler)
