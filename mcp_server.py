...
class IngredientReplacement(TypedDict):
    item: str
    quantity: dict[str, int] | int | float | str
    unit: str
    assumptions: str
...
class IngredientChange(TypedDict):
    ...
    assumptions: NotRequired[str]
    quantity: NotRequired[dict[str, int] | int | float | str]
    unit: NotRequired[str]
    replace_with: NotRequired[list[IngredientReplacement]]
...
def meal_concierge_recipe_delivery(
    ...
    return rpc("recipe_delivery", language=language, action=action, request_id=request_id, delivery_requested=delivery_requested,
...
@server.tool(
    ...
    description=(
        "Optional language on save freezes the output language; get can select a read-only presentation override. Missing variants report fallback. Choose a coherent menu using culinary judgment and the household profile. Call plan with planner_input.selection_mode=agent, chronological dates and one exact ordered candidate per cooking/source date. The service preserves this order and checks hard restrictions, dates and explicit strict_targets; cooldown is visible variety guidance for agent selection; saved numeric minima remain visible advisory goals in agent mode. Use ranked mode or omit selection_mode for legacy ranking and automatic discovery when candidates are omitted. Save only the unchanged save_ref as planner_ref. Existing-menu actions use the exact menu_ref={menu_id,revision,digest}; never split identity into top-level ID/revision fields. For a requested distinct whole new menu during a pending purchase, read menu get, plan, then save the unchanged save_ref as planner_ref with the exact current menu_ref; the pending purchase and cart stay frozen. For targeted remaining-slot changes to a separate menu, use replan_prepare/replan_apply even while an unrelated order change or payment recovery waits; the service checks the frozen purchase menu or exact existing-order snapshot, slot owners and usage under its state lock. A linked or unidentified menu, or pending cancellation, still requires the protected operation to resolve. Neither menu action retries payment or prepares cart goods; never confirm payment merely to unlock planning. Ranked planning retains explicit cooldown overrides; agent-selected repeats and exact slot edits need no variety approval. The planner returns bounded selections and source/unknown diagnostics and changes no cart. Known allergy/never-buy conflicts require alternatives. Honor ordinary preferences when choosing recipes: resolve obvious conflicts before save, adapting the existing dish first when requested. For diet.leafy_green_days, assess each exact dinner recipe with facts.leafy_green: source=explicit, assessment=substantial|does_not_count|unknown, ingredient_indices for substantial listed leafy ingredients, and a brief basis. Treat the agent judgment as culinary assessment; the service verifies indexed required mass lines and portions, then counts possible dinners against the inclusive weekly range. Do not count herbs, garnish or the entire weight of a mixed product as leaves, and do not invent gram conversions. Missing or unsupported evidence remains unknown. Advisory means model responsibility, not permission to knowingly ignore preferences. For every agent-selected recurring batch dish, supply facts.batch_guidance with a recipe/date-specific basis, suitability, storage and reheating assessment; unknown guidance must remain explicit. The model is the author of this guidance, and unsuitable dishes need alternatives. Use edit_slots with exact menu_ref, stable idempotency_key and ordered edits to add, replace, remove or move future meal occurrences. Fresh add needs date, meal_type, portions, exact reference and optional leafy_green assessment. Replace needs slot_id and exact reference; remove needs slot_id; move needs slot_id and changed date, type or portions. A linked later serving uses source_slot_id or source_edit_index of an earlier fresh add, date and portions; one preparation shops once for the sum of its structured servings. Reversible edits need no variety/cooldown approval. Source rights, dietary exclusions, recorded slots and protected orders remain binding. Product/cart changes are separate. Exact replan and batch apply arguments remain opaque and replay-safe; preserve actual history and never invent consent, safety facts or source evidence."
    ),
)
def meal_concierge_menu(
    action: Literal["get", "assess", "plan", "save", "add_slot", "edit_slots", "resolve_handoff", "clear", "lock", "replan_prepare", "replan_apply", "batch_prepare", "batch_apply"] = "get",
    ...
    edits: Annotated[list[dict[str, Any]], Field(min_length=1, max_length=31)] | None = None,
    idempotency_key: str | None = None,
    view_offset: Annotated[int, Field(ge=0)] = 0, view_limit: Annotated[int, Field(ge=1, le=20)] = 10,
...
@server.tool(description="For an explicit Oda payment-method change, use switch_payment with the current confirmation_id and checkout_payment={method:vipps} or {method:saved_card}. For an existing new order it reconciles the old attempt and inspects the merchant’s current same-order payment review; an unknown local closure remains recorded. Additions require their separate verified target and closure. An explicit abort observes or closes only the exact old request; an unknown closure remains in the journal. A new unpaid order can still inspect the merchant’s current same-order payable review. Addition payment replacement keeps its verified closure or failure requirement. For an explicit same-order new-order cancellation while card 3D Secure or Vipps is unresolved, orders cancel_prepare returns the current confirmation for checkout abort_payment. For an exact existing-order addition, native order cancellation can proceed while payment is uncertain; the original receipt or exact accepted addition and both journals must still match. Abort durably fences one native closure attempt and preserves the checkout journal; resume the same abort to observe an unknown result without another click. A paid result must reconcile the purchase, while positive closure allows a fresh exact same-order cancellation review. Only verified merchant cancellation archives the checkout; refund and authorization release remain unknown. Unknown outcomes remain journalled. For the same new order, inspect a fresh merchant payment review; never repeat an old cancellation or dispatch click. A payment already accepted is reconciled without replacement. Confirm only the returned fresh review within the existing authorization. For a manual Oda/Mathem new-checkout identity mismatch, inspect the returned bounded expected/actual product fields and digest. The model may resolve cosmetic semantic equivalence by preparing the same cart with identity_review={digest,decisions:[{expected_index,actual_index,reason}]}; give a concrete reason for each mapping. Do not equate different products, pack sizes, quantities, or conflicting IDs. The service binds a one-to-one mapping to the current rows, account, payment, delivery and amounts and verifies it again before payment. A stale digest requires a fresh same-cart review. This proof applies only to this checkout, never global preferences, additions, recovery or automatic checkout. When the owner explicitly says this exact Oda order was paid manually, reconcile its current confirmation with owner_payment_completed=true; verify merchant order details and report owner completion without claiming Vipps approval or settled bank charge. Prepare shows actual final-product dietary findings. Include them in the existing final confirmation; After the user reviews the current findings, pass summary.dietary_assessment.assessment_digest as dietary_review_digest together with this confirmation_id. Do not copy long lists of finding IDs. The legacy dietary_review list remains supported. Unknown ordinary preferences are advisory; actual deviations are shown once. Known allergy/never-buy conflicts cannot proceed. Automatic uncertainty requires exact accepted profile permission and its notice. Send notice payload only when dispatch=true through the existing authorized native route, record actual sender result via notice_result, then resume the same confirmation/key/occurrence without a user reply. Failed/unknown notices do not pass. Reconciliation returns the actual result notice and supported correction limits; send/ack it without replaying payment. Prepare, confirm, submit under configured standing authorization, or reconcile a new checkout or active existing-order change. Managed auto requires the exact occurrence and scheduler from schedule due. auto handles each due cart_ready or auto_checkout occurrence; cart_ready never submits payment and its returned occurrence must be carried into a later manual prepare. submit requires one stable idempotency_key per explicit order intent; reuse it only for that same uncertain call and use a new key for a later intent. A preview or prepare never submits. An actual MENY Vipps payment request requires the user's phone approval. An existing-order update may instead return an authenticated same-order receipt without another phone approval step; use the actual submit/reconcile result, never the button label. Mathem supports guarded SEK saved-card checkout with a configured dedicated browser. Without an available browser, prepare returns the manual cart link. Failed login, account/address or saved-card checks stop checkout and require attention before a fresh review. Mathem additions require change_begin for the exact modifiable order; guarded checkout inherits its original address/delivery and charges only the reviewed added goods. Guarded cancellation uses a fresh exact-order review. For a delivery-only change at any provider, begin the exact order edit with delivery_only=true, preserve the original goods, select the requested window and prepare a fresh full-total review. summary.delivery_change separates original/new totals, difference and payable amount. A concrete requested window authorizes unchanged or lower verified full total even under fresh policy. A higher total needs the bound explicit max_total_ore or one new approval of this exact window/difference/total; after that approval use confirm with delivery_price_approved=true. Generic standing policy is not price-increase authority. Missing exact full totals or unsupported merchant controls remain manual; never infer a refund. Reconcile the original attempt without a second dispatch. Unconfirmed payment keeps the original attempt; never restage goods or retry payment from an error. Workflow status refers to the active recovery child when present: confirm its fresh confirmation_id under the existing policy if prepared, or reconcile that same ID if dispatched or uncertain. Tell the user to approve Vipps only when the exact attempt positively records a sent request; saved-card and contextless legacy attempts do not prove a phone request. For a legacy Oda/Vipps payment-started order with no recorded request context or dispatch timestamp, an owner report of no Vipps request or manual payment can accompany read-only exact-order recovery prepare: pass recovery=true, the original confirmation_id, exact order_id, vipps_request_not_received=true, and the requested existing checkout_payment method (saved_card or vipps). The service must independently bind the same order, account, goods, delivery and amount; the report alone never authorizes retry or journal deletion. For an identified unpaid Oda/Mathem new order, or a failed Mathem addition with its original retry target retained, prepare with recovery=true reviews the same merchant payment without paying. For exact Oda/Vipps recovery, a coarse paid_and_modifiable or paid_and_not_modifiable tracking result may be treated as a conflict only after the owner identifies that exact order as Betaling påbegynt and reports no request in their Vipps app. For this recovery supply order_id with recovery=true, the original confirmation_id and vipps_request_not_received=true. Ordinary prepare also accepts order_id when it exactly matches the active Oda change. The browser must independently verify the exact payment-started order page and receipt, the unique post-checkout provider identity, and a direct same-order recovery review reproducing the frozen order, account, goods, delivery and payable amount with the selected existing payment method. An exact Betal link is accepted but is not required while that direct review remains available; other tracking states remain blocked. If an exact recovery stops before recording any request context or dispatch timestamp and the owner still received nothing, reconcile that recovery's fresh confirmation with vipps_request_not_received=true; only the same unpaid order or this narrowly verified payment-started tracking conflict plus the exact retry review may classify it as not sent. This report is replay-safe and the resulting fresh recovery prepare carries that evidence forward. A user report alone is insufficient, and a sent, dispatching or otherwise unresolved recorded Vipps request cannot be resent under its old confirmation; inspect a fresh merchant review only for the same identified unpaid new order. If that exact recovery is positively recorded not_sent, the original attempt also has no dispatch timestamp, and its exact order page is stuck at Betaling påbegynt without an actionable retry, abandon_unpaid with the same confirmation_id, order_id and current vipps_request_not_received=true may release only the local journal for one fresh checkout. The provider API must still identify the exact entry as unpaid or one of the known conflicting paid statuses; both observations are retained. The old provider entry remains payment-started, is durably fenced by order ID and must not be recovered later. Scheduled attempts, dispatched requests and other providers stay blocked. For additions, show any separately returned merchant_summary_total alongside the actual amount due; do not confuse the overview with the payment-button amount. Optional checkout_payment chooses an existing supported method for recovery or a manual Oda new order/addition prepare; for a manual existing-order delivery change it can select an existing saved card. Global setup is unchanged. Oda additions inherit the exact original account, address and delivery and pay only the positive reviewed delta. Select the method during prepare; confirm only verifies the frozen method. A dispatched addition payment remains locked to its same order and request: reconcile before any further action; a paid original order alone does not prove that the added goods were accepted. Oda supports selecting Vipps or an existing saved card before dispatch. After a payment request was sent or may have been sent, explicit switch_payment on a new order reconciles the old attempt and inspects the current merchant payment review for that same order. An unknown old outcome stays journalled. An addition requires its own verified payment target and terminal evidence. Confirm only its fresh recovery confirmation_id after the review is authorized. Reconcile a dispatched recovery without another payment. If its tracking result says paid, reconcile the exact completed Vipps approval with vipps_approval_completed=true, or the owner’s explicit manual-payment report with owner_payment_completed=true with the fresh recovery confirmation. An expired Vipps page still blocks confirmation. For an explicit cancellation of the current Oda order with unresolved payment, use orders cancel_prepare. For a new Oda order, checkout abort_payment observes the exact old request; an unknown closure remains journalled but does not bar inspecting the merchant’s current same-order payment review. Never create a replacement order. Addition payments retain separate closure requirements. An exact existing-order addition can proceed to merchant cancellation while its payment remains uncertain when the original receipt or exact accepted addition, account, and both journals still match; never retry that addition payment. Only verified merchant cancellation closes the checkout journal, and bank refund or authorization release stays unknown. After a payment dispatch, a new-order same-order recovery review may inspect the current merchant payment page even when local status is unknown. Confirm only that fresh exact review; additions retain their separate terminal evidence rules. Old confirmations cannot act on a newer attempt. Never run an automatic payment retry loop. Oda/Mathem 3D Secure keeps the same payment page open. When authentication_required is true, authenticate with its exact confirmation_id can select the supported Bank Norwegian Appen method once; it never enters inputs or approves the payment. The user approves in their own bank app, then reconcile the same confirmation. Never receive/read/fill BankID passwords. An unavailable chooser leaves the old attempt unresolved; inspect a fresh merchant payment review before any different action on the same identified new order.")
def meal_concierge_checkout(action: Literal["prepare", "confirm", "submit", "reconcile", "switch_payment", "abort_payment", "abandon_unpaid", "authenticate", "auto", "notice_result"] = "prepare", recovery: bool = False, checkout_payment: dict[str, Any] | None = None, identity_review: dict[str, Any] | None = None, order_id: str | None = None, vipps_request_not_received: bool | None = None, vipps_approval_completed: bool | None = None, owner_payment_completed: bool | None = None, occurrence: str | None = None, confirmation_id: str | None = None, idempotency_key: str | None = None, scheduler: dict[str, Any] | None = None, dietary_review: list[str] | None = None, dietary_review_digest: str | None = None, weekly: bool = False, delivery_price_approved: bool | None = None, notice_token: str | None = None, send_outcome: Literal["sent", "not_sent", "unknown"] | None = None, sender_receipt: str | None = None) -> dict[str, Any]:
    return rpc("checkout", action=action, **({"recovery": True} if recovery else {}), **({"checkout_payment": checkout_payment} if checkout_payment is not None else {}), **({"identity_review": identity_review} if identity_review is not None else {}), **({"order_id": order_id} if order_id is not None else {}), **({"vipps_request_not_received": vipps_request_not_received} if vipps_request_not_received is not None else {}), **({"vipps_approval_completed": vipps_approval_completed} if vipps_approval_completed is not None else {}), **({"owner_payment_completed": owner_payment_completed} if owner_payment_completed is not None else {}), occurrence=occurrence, confirmation_id=confirmation_id, idempotency_key=idempotency_key, scheduler=scheduler, **{k: v for k, v in {"dietary_review": dietary_review, "dietary_review_digest": dietary_review_digest, "weekly": weekly if weekly else None, "delivery_price_approved": delivery_price_approved, "notice_token": notice_token, "send_outcome": send_outcome, "sender_receipt": sender_receipt}.items() if v is not None})
