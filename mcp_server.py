...
@server.tool(
    ...
    description=(
        "Choose a coherent menu using culinary judgment and the household profile. Call plan with planner_input.selection_mode=agent, chronological dates and one exact ordered candidate per cooking/source date. The service preserves this order and checks hard restrictions, cooldown, dates and explicit strict_targets; saved numeric minima remain visible advisory goals in agent mode. Use ranked mode or omit selection_mode for legacy ranking and automatic discovery when candidates are omitted. Save only the unchanged save_ref as planner_ref. Existing-menu actions use the exact menu_ref={menu_id,revision,digest}; never split identity into top-level ID/revision fields. For a distinct whole new menu draft during a waiting-payment state that permits menu saving, read menu get, plan, then save the unchanged save_ref as planner_ref together with the exact current menu_ref returned by get; this leaves the pending purchase and cart frozen. Active provider handlers, prepared recovery reviews, order changes and cancellations must follow their status/recovery paths before saving; never confirm payment merely to unlock planning. For ordinary same-week remaining-slot changes, use replan_prepare/replan_apply so retired planned slots do not block themselves; a targeted replan waits for pending checkout to resolve. planner_input.cooldown_overrides is only for an explicitly requested historical repeat. The planner returns bounded selections and source/unknown diagnostics and changes no cart. Known allergy/never-buy conflicts require alternatives. Honor ordinary preferences when choosing recipes: resolve obvious conflicts before save, adapting the existing dish first when requested. Advisory means model responsibility, not permission to knowingly ignore preferences. Use add_slot for an explicitly requested dated extra meal/course. Exact replan and batch apply arguments remain opaque and replay-safe; preserve actual history and never invent consent, safety facts or source evidence."
    ),
)
