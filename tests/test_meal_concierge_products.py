...
class ProductPlannerTests(unittest.TestCase):
    ...
    def test_digest_bound_continuation_retains_unaffected_and_invalidates_shared_group(self):
        value = menu(
            {"item": "egg", "quantity": 1, "unit": "count"},
            {"item": "eggeplomme", "quantity": 2, "unit": "count"},
            {"item": "mel", "quantity": 500, "unit": "g"},
        )
        requirements = menu_requirements(value)[0]
        by_item = {row["item"]: row for row in requirements}
        egg = product(28866, "Egg frittgående 12-pk", 12, "count", [option(500)])
        flour = product(110, "Siktet hvetemel", 1000, "g", [option(1900)])
        flour_alternative = product(
            111, "Hvetemel 1 kg", 1000, "g", [option(2100)],
        )
        observations = {
            by_item["egg"]["requirement_id"]: observation("egg", [egg]),
            by_item["eggeplomme"]["requirement_id"]: observation("eggeplomme", [egg]),
            by_item["mel"]["requirement_id"]: observation(
                "mel", [flour, flour_alternative],
            ),
        }
        shared_ids = [
            by_item[item]["requirement_id"] for item in ("egg", "eggeplomme")
        ]
        shared = {
            "requirement_ids": shared_ids,
            "package_count": 1,
            "quantity_basis": "One observed 12-egg package covers both exact needs.",
            "authorized_by": "current_user",
        }
        approvals = [{
        ...
        approvals.append({
            "requirement_id": by_item["mel"]["requirement_id"],
            "candidate_refs": [110, 111],
        })
        menu_ref = {
            "menu_id": "menu_fixture", "revision": 7, "digest": "b" * 64,
        }
        binding = {"kind": "saved_menu", "menu_ref": menu_ref}
        prior = build_product_plan(
            provider="oda", binding=binding, menu=value,
            observations=observations, candidate_approvals=approvals,
        )
        self.assertEqual(prior["status"], "prepared")
        selection_digest = product_selection_digest(prior)

        retained_only = prepare_product_plan_continuation(
            prior, menu_ref=menu_ref, prior_selection_digest=selection_digest,
            affected_requirement_ids=[shared_ids[0]], candidate_approvals=[],
        )
        self.assertEqual(
            retained_only["invalidated_requirement_ids"], sorted(shared_ids)
        )
        self.assertEqual(
            retained_only["retained_requirement_ids"],
            [by_item["mel"]["requirement_id"]],
        )
        self.assertEqual(
            [row["requirement_id"] for row in retained_only["candidate_approvals"]],
            [by_item["mel"]["requirement_id"]],
        )

        continuation = prepare_product_plan_continuation(
            prior, menu_ref=menu_ref, prior_selection_digest=selection_digest,
            affected_requirement_ids=[shared_ids[0]],
            candidate_approvals=approvals[:2],
        )
        revalidated_observations = {
            by_item["egg"]["requirement_id"]: observation(
                "egg", [egg], observed_at="2026-09-04T13:00:00+00:00",
            ),
            by_item["eggeplomme"]["requirement_id"]: observation(
                "eggeplomme", [egg], observed_at="2026-09-04T13:00:00+00:00",
            ),
            by_item["mel"]["requirement_id"]: observation(
                "mel", [flour, flour_alternative],
        ...
        changed_unaffected = build_product_plan(
            provider="oda", binding=binding, menu=value,
            observations={
                **revalidated_observations,
                by_item["mel"]["requirement_id"]: observation(
                    "mel", [repriced_flour, flour_alternative],
        ...
        self.assertEqual(
            changed_unaffected["requirements"][2]["selection"]["products"][0]["product_ref"],
            111,
        )
        ...
        stale_facts = build_product_plan(
            provider="oda", binding=binding, menu=value,
            observations={
                **revalidated_observations,
                by_item["mel"]["requirement_id"]: observation(
                    "mel", [unavailable_flour], observed_at="2026-09-04T13:00:00+00:00",
                ),
            },
            candidate_approvals=continuation["candidate_approvals"],
        )
        ...
        with self.assertRaisesRegex(HouseholdError, "menu revision changed"):
            prepare_product_plan_continuation(
                prior, menu_ref={**menu_ref, "revision": 8},
                prior_selection_digest=selection_digest,
                affected_requirement_ids=[shared_ids[0]], candidate_approvals=[],
            )
        with self.assertRaisesRegex(HouseholdError, "selection digest changed"):
            prepare_product_plan_continuation(
                prior, menu_ref=menu_ref, prior_selection_digest="0" * 64,
                affected_requirement_ids=[shared_ids[0]], candidate_approvals=[],
            )
